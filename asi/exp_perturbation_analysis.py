...
def hellinger_norm_sqrt(p, q):
    return norm(np.sqrt(p) - np.sqrt(q)) / np.sqrt(2) 

def hellinger_euclidean(p, q):
    return euclidean_distance(np.sqrt(p), np.sqrt(q)) / np.sqrt(2) 

def hellinger_native(p, q):
...
def pearson_correlation(x, y):
    ...
    Notes:
        ...
        between two arrays. It ranges from -1 to 1, where 1 indicates a perfect
        positive linear correlation, -1 indicates a perfect negative linear
...
def deletion_sequence(time_series_sample, attribution_values, threshold, value=0, length=1, larger=True):
    tmp_ts = copy_data(time_series_sample)
...
def insertion_sequence(time_series_sample, attribution_values, threshold, value=0, length=1, larger=True):
    tmp_ts = copy_data(time_series_sample)
    
    base = np.full(tmp_ts.shape, value)
    ...
    return base * mask + (1 - mask) * tmp_ts, 1 - mask
...
def ood_h(**kwargs):
    if 'gts' in kwargs:
        ...
        return np.abs(kwargs['ts']) * 100000
...
def ood_l(**kwargs):
    if 'gts' in kwargs:
        ...
        return -1 * np.abs(kwargs['ts']) * 100000
